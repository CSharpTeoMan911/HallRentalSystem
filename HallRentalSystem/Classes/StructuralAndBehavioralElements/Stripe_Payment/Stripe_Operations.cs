...
{
    public class Stripe_Operations : CRUD_Strategy<Booking_Parameters, Payment_Parameters, string, string>
    {
        ...
        public async Task<ReturnType?> Get<ReturnType>(Payment_Parameters? data)
        {
            ...
            try
            {
                if (data != null)
                {
                    ...
                    {
                        string? log_in_session_key_verification_result = await Shared_Data.log_in_session.Get<string>(data.log_in_session_key);
                        ...
                        {
                            if (data.payment_intent_id_database_key != null)
                            {
                                if (Firebase_Database.firebaseClient != null)
                                {
                                    ...
                                    {
                                        string serialised_pending_transactions = await database_reference.OrderByKey().EqualTo(data.payment_intent_id_database_key).OnceAsJsonAsync();
                                            ...
                                            if (deserialised_pending_transaction != null)
                                            {
                                                ...
                                                if (search_result != null)
                                                {
                                                    if (search_result.Status == "succeeded")
                                                    {
                                                        ...
                                                        if (Firebase_Database.firebaseClient != null)
                                                        {
                                                            await Firebase_Database.firebaseClient.Child("Pending_Transactions/Pending_Transaction_ID").Child(data.payment_intent_id_database_key).DeleteAsync();
                        ...
                        {
                            result = log_in_session_key_verification_result;
                        }
                    }
                    else
                    {
                        await Shared_Data.log_in_session.Delete<string>(data.log_in_session_key);
                        result = "Invalid log in session key";
                    }
